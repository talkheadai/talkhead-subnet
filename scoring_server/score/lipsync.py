...
def compute_lip_sync_score(video_path: Path) -> float:
    ...
    (
        ...
        audio_path=None,  # extract audio from video
    )
    ...
    # From the docs: good sync often shows confidence ~4.5+.
    # Map confidence in [0, 6] -> [0,1]
    print(f"best_confidence: {best_confidence}")
    print(f"best_min_dist: {best_min_dist}")
    conf = float(best_confidence)
    S_conf = max(0.0, min(conf / 6.0, 1.0))
