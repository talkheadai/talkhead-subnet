...
def metric_quality(video_path: Path) -> Tuple[MetricResult, Optional[float]]:
    ...
    try:
        ...
        esr = RealESRGANer(
            scale=4,
            model_path="https://huggingface.co/ai-forever/Real-ESRGAN/resolve/main/RealESRGAN_x4.pth",
            model=model,
            tile=0,
            tile_pad=10,
            pre_pad=0,
        ...
        return MetricResult(
            ...
            raw=None,
            available=False,
            detail=f"failed to init Real-ESRGANer: {exc}",
        ), None
    ...
    for frame in frames:
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        # ESRGAN enhance expects RGB uint8
        try:
            sr, _ = esr.enhance(rgb, outscale=4)
        except Exception:
            # fall back to original if enhance fails
            sr = rgb
        sr = np.clip(np.array(sr), 0, 255).astype(np.uint8)
        h, w, _ = rgb.shape
        base = cv2.resize(rgb, (w * 4, h * 4), interpolation=cv2.INTER_CUBIC)
